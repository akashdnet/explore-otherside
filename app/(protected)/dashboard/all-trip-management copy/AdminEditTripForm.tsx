...
import { updateTripAdmin } from "@/actions/trip";
...
export default function AdminEditTripForm({ trip, onSuccess }: EditTripFormProps) {
    ...
    const onSubmit = async (values: TEditTrip) => {
        ...
        try {
            ...
            const res = await updateTripAdmin(trip._id, formData);
            if (res?.success) {
                toast({
                    title: "Success",
                    description: "Trip updated successfully (Admin)",
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <Button type="submit" className="w-full" disabled={isLoading}>
                    {isLoading ? "Updating..." : "Update Trip (Admin)"}
                </Button>
            </form>
        </Form>
    );
